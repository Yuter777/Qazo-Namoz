<template>
  <div class="qn-page fade-in">
    ...
    <div class="desktop-profile-grid">
      <!-- Left col -->
      <div>
        <div class="qn-section-label" style="padding-left: 0">{{ t('profile.language') }}</div>
      ...
      <!-- Right col -->
      <div>
        <div class="qn-section-label" style="padding-left: 0">{{ t('profile.settings') }}</div>
        <div class="qn-card" style="margin: 0">
          <div class="toggle-row">
            <div>
              <div class="toggle-label">{{ t('profile.darkMode') }}</div>
            </div>
            <button class="qn-toggle" :class="{ on: uiStore.isDark }" @click="uiStore.toggleTheme()" />
          </div>
          <div class="toggle-row">
            <div>
            ...
            <button class="qn-toggle" :class="{ on: notif }" @click="notif = !notif" />
          </div>
        </div>
    ...
    <div class="mobile-only-profile">
      ...
      <div class="qn-card" style="margin: 0 20px">
        <div class="toggle-row">
          <div><div class="toggle-label">{{ t('profile.darkMode') }}</div></div>
          <button class="qn-toggle" :class="{ on: uiStore.isDark }" @click="uiStore.toggleTheme()" />
        </div>
        <div class="toggle-row">
          <div>
...
.profile-name  { font-size: 20px; font-weight: 700; color: var(--text1); }
...
.lang-pill.active { background: var(--teal-light); border-color: var(--teal); color: var(--teal-dark); }
...
.goal-label { flex: 1; font-size: 15px; color: var(--text1); }
.goal-input {
...
@media (min-width: 768px) {
  .profile-header { padding: 32px 32px 20px; }
