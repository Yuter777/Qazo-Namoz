  ...
  <div class="qn-page fade-in">
    ...
    <div class="qn-hero">
      <div style="display: flex; justify-content: space-between; align-items: flex-start; position: relative; z-index: 1">
        <div>
          <div style="font-size: 13px; opacity: 0.8; font-weight: 400">{{ t('dashboard.greeting') }}</div>
          <div style="font-size: 22px; font-weight: 700; margin-top: 2px; letter-spacing: -0.3px">{{ displayName }}</div>
    ...
    <div class="stat-grid">
      <div class="stat-card">
        <div class="stat-num" style="color: var(--teal)">{{ totalRemaining }}</div>
      ...
      <div class="stat-card">
        <div class="stat-num" style="color: var(--amber)">{{ remainingToday }}</div>
  ...
  setup(props) {
    ...
    return () => h('div', [
      h('div', { style: 'display:flex;flex-direction:column;align-items:center;padding:24px 20px 16px' }, [
        ...
        h('div', { style: 'font-size:13px;color:var(--text2);margin-top:10px' },
        ...
        PRAYERS.map((p) => {
          ...
          return h('div', { key: p, style: 'display:flex;align-items:center;gap:10px' }, [
            h('div', { style: 'font-size:13px;font-weight:500;color:var(--text1);width:80px;flex-shrink:0;display:flex;align-items:center;gap:6px' }, [
              ...
              h('span', { style: 'font-size:12px' }, t(`prayers.${p}`).split(' ')[0]),
            ]),
            h('div', { class: 'qn-progress-bar-track' }, [
              h('div', { class: 'qn-progress-bar-fill', style: `width:${barPct}%;background:${PRAYER_COLORS[p]}` }),
            ]),
            h('div', { style: 'font-size:12px;color:var(--text2);width:32px;text-align:right;flex-shrink:0' }, `${barPct}%`),
          ])
        })
      ),
    ])
...
.stat-num   { font-size: 24px; font-weight: 700; color: var(--text1); letter-spacing: -1px; }
